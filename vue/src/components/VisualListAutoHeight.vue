<template>
    <div ref="container" class="container" @scroll="handleScroll($event)">
        ...
        <div class="list-wrapper" :style="{ transform: getTransform }">
            <div class="card-item" v-for="item in renderList" :key="item.index" ref="itemRefs" :data-index="item.index">
                <span style="color: red">{{ item.index }}           <img width="200" :src="item.imgUrl" alt="" />
                </span>
                {{ item.value }}
            </div>
        </div>
